...
(function () {
  ...
  window.VERSION_JUEGO = 'v0.1.0-beta.1';
  ...
  // La recarga limpia el estado de la partida; el guardado por perfil persiste.
  const btnNoclip = document.getElementById('btn-noclip-menu');
  ...
  document.getElementById('ajustes-version').textContent =
    `BACKROOMS NOCLIP OFFLINE ${window.VERSION_JUEGO}`;
  ...
  // ?local=1 y el antiguo ?online=1 arrancan siempre la simulación local offline
  ...
  function conectarAlServidor(btnOrigen) {
    ...
    const btn = btnOrigen || btnOffline;
    ...
    btn.textContent = 'ATRAVESANDO LA REALIDAD…';
    ...
    esperaConexion = setInterval(() => {
      if (Net.activo) {
        ...
        restaurar();
        ocultarCaida(true);
        errNet.textContent = Net.ultimoError || 'No se pudo iniciar la simulación local.';
  ...
  function playMenuMusic() {
    ...
    if (document.getElementById('btn-offline').disabled) return;
  ...
  function refreshTitle() {
    ...
    if (saveData && p) {
      ...
      btn.textContent = `Continuar partida (${saveData.levelId})`;
      btn.onclick = () => { window.MODO_LOCAL = true; conectarAlServidor(btn); };
  ...
  // Inicio exclusivo en el motor local de la aplicación.
